function PayGapByJob2017() {
  ...
  this.draw = function () {
    ...
    for (i = 0; i < this.data.getRowCount(); i++) {
      ...
      // Calculate distance between mouse and the ellipse
      var distanceEllipseMouse = dist(map(propFemale[i], propFemaleMin, propFemaleMax,
              this.pad_, width - this.pad_),
          map(payGap[i], this.payGapMin, this.payGapMax,
              height - this.pad_, this.pad_),
          mouseX, mouseY);
      // This is to display each data values when mouse is on each ellipse
      if (distanceEllipseMouse <= map(numJobs[i], numJobsMin, numJobsMax,
          this.dotSizeMin, this.dotSizeMax)) {
        fill(193, 225, 193, 50);
        stroke(0);
        ...
        text(round(propFemale[i], 4), width / 2 - 240, height / 2 + 90);
        text(' Pay Gap : ', width / 2 - 400, height / 2 + 110);
        text(Math.round(payGap[i] * 100) / 100, width / 2 - 290, height / 2 + 110);
        noFill();
      }
    }
  };
  this.addAxes = function () {
    ...
    fill(0);
    noStroke();
    text('Female Proposition', width - 115, height / 2 + 10 );
    text('*Circle Size = Number of Jobs', 100, 500);
  };
  // This to color each ellipse based on each job's pay gap.
