function PayGapTimeSeries() {
  ...
  this.draw = function () {
    ...
    rect(this.layout.leftMargin,
        ...
        this.layout.bottomMargin - marginSize);
    ...
    for (var i = 0; i < this.data.getRowCount(); i++) {
      ...
      // Added Red dots for the peak of every year
      strokeWeight(11);
      stroke(255, 0, 0, 150)
      ...
      strokeWeight(1);
      stroke(0)
      // distance between current data point and mouseX, Y
      var distanceCurrentMouse = dist(this.mapYearToWidth(current.year),
          this.mapPayGapToHeight(current.payGap),
          mouseX, mouseY);
      // displaying data values for each of the year 
      if (distanceCurrentMouse < 9) {
        fill("black");
        text(current.year, this.mapYearToWidth(current.year), this.mapPayGapToHeight(current.payGap) + 30);
        text(current.payGap.toFixed(2), this.mapYearToWidth(current.year),
            this.mapPayGapToHeight(current.payGap) + 45);
      }
