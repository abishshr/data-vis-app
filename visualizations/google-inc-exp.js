        ...
        for (var i = 0; i < this.data.getRowCount(); i++) {
            ...
            if (previous != null) {
                ...
                if (mouseX < this.mapYearToWidth(current.year) && mouseX > Income_width && mouseY < this.layout.bottomMargin && mouseY > this.mapAmountToHeight(current.Income)) {
                    this.informationBox = new InformationBox(leftMargin = this.layout.leftMargin,
                        topMargin = this.layout.topMargin, Expenditure = current.Expenditure ,Income = current.Income ,year = current.year)
